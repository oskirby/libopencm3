...
void
rcc_css_hse_int_clear(void)
{
	RCC_CICR = RCC_CICR_CSSHSEC;
}

void
rcc_css_lse_int_clear(void)
{
	RCC_CICR = RCC_CICR_CSSLSEC;
}

int
rcc_css_hse_int_flag(void)
{
	return ((RCC_CIFR & RCC_CIFR_CSSHSEF) != 0);
}

int
rcc_css_lse_int_flag(void)
{
...
void rcc_wait_for_sysclk_status(enum rcc_osc osc)
{
	...
	case RCC_HSI16:
		while (((RCC_CFGR >> RCC_CFGR_SWS_SHIFT) & RCC_CFGR_SWS_MASK) !=
			RCC_CFGR_SWS_SYSCLKSEL_HSI16CLK);
		break;
	case RCC_MSI:
		while (((RCC_CFGR >> RCC_CFGR_SWS_SHIFT) & RCC_CFGR_SWS_MASK) !=
			       ...
			       uint32_t divisor)
{
	...
	reg32 |= (source << RCC_CFGR_PLLSRC_SHIFT);
...
void rcc_clock_setup_hsi(const struct rcc_clock_scale *clock)
{
	...
	while (PWR_CSR & PWR_CSR_VOSF)
		;
	rcc_set_sysclk_source(RCC_CFGR_SW_SYSCLKSEL_HSI16CLK);
